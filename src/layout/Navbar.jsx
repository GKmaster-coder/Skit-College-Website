...
const menuItems = [
  { name: 'Home' },
  { name: 'About', sub: ['Overview', 'Our History', 'Vision & Mission'] },
  { name: 'Course', sub: ['All Courses', 'Admissions'] },
  { name: 'Faculty' },
  { name: 'Gallery' },
  { name: 'Performer' },
  { name: 'Download' },
...
const NavButton = ({ item, activeDropdown, setActiveDropdown }) => {
  ...
  return (
    <li ref={ref} className="relative group">
      <button
        onClick={() => item.sub ? setActiveDropdown(isOpen ? null : item.name) : null}
...
const DropItem = ({ label }) => (
  <li className="px-5 py-2.75 text-[11.5px] font-semibold text-gray-500 uppercase tracking-wider border-b border-gray-100 cursor-pointer hover:bg-[#eef5d8] hover:text-[#6a9420] transition-colors duration-150">
    {label}
  </li>
);
